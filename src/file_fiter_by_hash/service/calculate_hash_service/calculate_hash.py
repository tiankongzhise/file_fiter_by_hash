...
def calculate_file_hash(params: HashParams) -> HashResult:
    try:
        ...
        for alg in params.algorithm:
            ...
            logger.info(message=f'文件 {params.item_path} 哈希值 {alg} 计算完成，哈希值为 {hash_result[alg]}')
    except Exception as e:
        logger.error(message=f'文件 {params.item_path} 哈希值 {params.algorithm} 计算失败，错误信息为 {str(e)}')
