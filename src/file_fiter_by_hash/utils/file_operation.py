...
def remove_to_pending_backup(folder_name: str|pathlib.Path):
    ...
    if move_result is True:
        logger.info(message=f"文件夹 {folder_name} 已成功移动到备份文件夹 {backup_dir}")
        return True
    else:
        logger.error(message=f"文件夹 {folder_name} 移动到备份文件夹 {backup_dir} 失败 → {move_result}")
        return False
...
def remove_to_processing(folder_name: str|pathlib.Path):
    ...
    if move_result is True:
        logger.info(message=f"文件夹 {folder_name} 已成功移动到待人工处理文件夹 {processing_dir}")
        return True
    else:
        logger.error(message=f"文件夹 {folder_name} 移动到待人工处理文件夹 {processing_dir} 失败 → {move_result}")
        return False
def remove_to_panding_delete(folder_name: str|pathlib.Path):
    ...
    if move_result is True:
        logger.info(message=f"文件夹 {folder_name} 已成功移动到待删除文件夹 {delete_dir}")
        return True
    else:
        logger.error(message=f"文件夹 {folder_name} 移动到待删除文件夹 {delete_dir} 失败 → {move_result}")
        return False
...
def delete_empty_folder(folder_name:str|pathlib.Path):
    ...
    if folder_name.exists() and folder_name.is_dir() and not any(folder_name.iterdir()):
        folder_name.rmdir()
        logger.info(message=f"空文件夹 {folder_name} 已成功删除")
        return True
    else:
        logger.error(message=f"文件夹 {folder_name} 不是空文件夹，无法删除")
        return False
...
def remove_to_local_duplicate(folder_name: str|pathlib.Path):
    ...
    move_result = move_file_by_pathlib(folder_name, delete_dir)
    if move_result is True:
        logger.info(message=f"文件夹 {folder_name} 已成功移动到本地待删除文件夹 {delete_dir}")
        return True
    else:
        logger.error(message=f"文件夹 {folder_name} 移动到本地待删除文件夹 {delete_dir} 失败 → {move_result}")
        return False
